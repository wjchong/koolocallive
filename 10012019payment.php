...
if( $_POST['wallet'] == "MYR" )
{
 ...
 if($balance['balance_myr'] < $amount)
	...
	if($flag == false)
	{
			...
			$order_data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT merchant_id FROM order_list WHERE id='".$o_id."'"));
	...
	if(isset($_POST['guest_id']))
{
	...
	$order_id=$_POST['guest_order_id'];
	 $order_data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT merchant_id FROM order_list WHERE id='".$order_id."'"));
			  $merchant_id=$order_data['merchant_id'];
...
// die;
			$order = explode("_",$_POST['wallet']);
			 ...
			 $wallet_c = $order['0'];
			 $wallet_oid = $order['1'];
			 $order_data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT merchant_id FROM order_list WHERE id='".$wallet_oid."'"));
			  $merchant_id=$order_data['merchant_id'];
