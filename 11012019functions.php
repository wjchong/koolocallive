...
if(isset($_POST['method']) && ($_POST['method'] == "getNoneImageProduct")){
    ...
    while ($row=mysqli_fetch_row($result)){
        $item = array("product_name"=> $row[2], "category"=> $row[3], 'type'=> $row[4], 'price'=> $row[5], 'remark'=>$row[8], 'id' => $row[0]);
...
if( isset( $_POST['method']) && ( $_POST['method'] == "getUnPrintedOrder" )  ) {
    ...
    $ref_result = mysqli_fetch_assoc(mysqli_query($conn, "SELECT name, id, order_print_setting, sst, gst FROM users WHERE id='".$id."'"));
